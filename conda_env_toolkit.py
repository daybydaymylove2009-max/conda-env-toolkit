...
class CondaManager:
    ...
    def analyze_failure_reason(package_name: str, stderr: str = "") -> str:
        ...
        # 先根据包名特征判断（优先级最高）
        # 1. 检查是否是系统库/开发包
        # 使用 set 提高查找效率，包含 300+ 常见系统库/开发包模式
        system_libs = {
            # GNU 库 / 编译器运行时
            'libasprintf', 'libgcc', 'libgomp', 'libstdcxx', 'libstdc++', 'libgfortran',
            'libquadmath', 'libatomic', 'libitm', 'libvtv', 'libssp',
            'libintl', 'libgettext', 'libgettextpo', 'gettext',
            # BLAS / LAPACK / 数学库
            'libblas', 'libcblas', 'liblapack', 'liblapacke', 'libopenblas',
            'libmkl', 'libmklml', 'libiomp5', 'libomp',
            # 图像 / 字体 / 图形 / X11
            'libfreetype', 'libfreetype6', 'libpng', 'libjpeg', 'libtiff',
            'libwebp', 'libgif', 'libraw', 'libopenjp2', 'libjbig',
            'libgl', 'libglu', 'libglew', 'libglfw', 'libsdl', 'libsdl2',
            'libopengl', 'libcups', 'libxv',
            'libx11', 'libxcb', 'libxau', 'libxdmcp', 'libxext', 'libxrender',
            'libxi', 'libxtst', 'libxinerama', 'libxrandr', 'libxfixes',
            'libxcursor', 'libxcomposite', 'libxdamage', 'libxft',
            'libfontconfig', 'libfribidi', 'libharfbuzz', 'libpango',
            'libcairo', 'libpixman', 'libgdk', 'libgtk', 'libatk',
            'libxkb', 'libxkbcommon', 'libxkbfile', 'libxshmfence',
            'libxss', 'libxt', 'libxmu', 'libxmuu', 'libxpm', 'libxres',
            'libxvmc', 'libxxf86dga', 'libxxf86vm', 'libxcb-keysyms',
            'libxcb-randr', 'libxcb-shape', 'libxcb-util', 'libxcb-xfixes',
            'libxcb-xinerama', 'libxcb-xinput', 'libxcb-xkb',
            # Wayland
            'libwayland-client', 'libwayland-cursor', 'libwayland-egl', 'libwayland-server',
            # 压缩 / 归档
            'libzlib', 'libz', 'libbz2', 'libbzip2', 'liblzma', 'liblzo',
            'liblz4', 'libzstd', 'libsnappy', 'libbrotli', 'libarchive',
            # 加密 / SSL / 安全
            'libssl', 'libcrypto', 'libopenssl', 'libgnutls', 'libgcrypt',
            'libnettle', 'libhogweed', 'libgmp', 'libmpfr', 'libmpc',
            'libisl', 'libtasn1', 'libp11', 'libidn2', 'libunistring',
            'libgpg-error', 'libgssglue',
            # 网络 / 协议
            'libcurl', 'libssh2', 'libgit2', 'libnghttp2', 'libcares',
            'libev', 'libuv', 'libevent', 'libthrift', 'libprotobuf',
            'libgrpc', 'libnanomsg', 'libzmq', 'libczmq', 'libnorm',
            # 数据库
            'libsqlite', 'libsqlite3', 'libpq', 'libmysql', 'libmariadb',
            'libhdf5', 'libnetcdf', 'libcdf', 'libgrib', 'libarrow',
            'libparquet', 'liborc', 'liblmdb',
            # 音视频
            'libpulse', 'libalsa', 'libjack', 'libportaudio', 'libsndfile',
            'libvorbis', 'libogg', 'libflac', 'libopus', 'libmp3lame',
            'libx264', 'libx265', 'libvpx', 'libtheora', 'libdav1d',
            'libaom', 'libsvtav1', 'libsvtav1enc', 'libvmaf', 'libass', 'libfdk',
            'libffmpeg', 'libav', 'libavcodec', 'libavformat', 'libavutil',
            'libswscale', 'libswresample', 'libvorbisenc', 'libvorbisfile',
            'libopenal', 'libopenmpt', 'libwildmidi', 'libtwolame', 'libudfread',
            'libvidstab', 'libvisual', 'libvncserver', 'libplacebo',
            'libsamplerate', 'librubberband', 'libsrt',
            # 文本 / 正则 / 解析
            'libxml2', 'libxslt', 'libexslt', 'libexpat', 'libpcre',
            'libpcre2', 'libonig', 'libicu', 'libjson', 'libyaml',
            'libmarkdown', 'libhunspell', 'libenchant', 'libtinyxml',
            'libxmlsec', 'libxmlsec1-openssl', 'liburiparser',
            # 系统 / 硬件抽象
            'libudev', 'libsystemd', 'libdbus', 'libhal', 'libusb',
            'libpci', 'libhwloc', 'libnuma', 'libnl', 'libibverbs',
            'librdmacm', 'libfabric', 'libpsm2', 'libpmi',
            'libdrm', 'libv4l', 'libv4lconvert', 'libvdpau', 'libva',
            'libva-drm', 'libva-x11', 'libvulkan', 'libwacom',
            # 内存分配器 / 性能
            'libjemalloc', 'libtcmalloc', 'libmimalloc', 'lib hoard',
            'libtbb', 'libtbbmalloc',
            # 调试 / 分析 / 异常处理
            'libelf', 'libdw', 'libunwind', 'libbacktrace',
            'libsanitizer', 'libtsan', 'libasan', 'libubsan',
            'liblsan', 'libmsan', 'libhwasan', 'libdwarf',
            # 其他常用系统库
            'libffi', 'libuuid', 'libblkid', 'libmount',
            'libnsl', 'libtirpc', 'librpc', 'libsocket',
            'libiconv', 'libcharset', 'libidn', 'libpsl',
            'libgssapi', 'libgssrpc', 'libkrb5', 'libk5crypto', 'libcom_err', 'libcom-err',
            'libldap', 'libsasl2', 'librtmp', 'libssh',
            'libzip', 'libmagic', 'libmagic-mgc', 'libacl', 'libattr',
            'libselinux', 'libsepol', 'libapparmor',
            'libcgroup', 'libseccomp', 'libaudit',
            'libcap', 'libcap-ng', 'libpam',
            'libreadline', 'libedit', 'libncurses', 'libncursesw', 'libtinfo',
            'libpopt', 'libproc', 'libuchardet', 'libtre',
            'libpaper', 'libpipeline',
            # Windows 特定
            'libwinpthread', 'libwiniconv', 'libwineditline',
            # 科学数据格式
            'libcfitsio', 'libwcs', 'libhealpix',
            'libgeos', 'libproj', 'libgdal', 'libogdi',
            'libkml', 'liblerc', 'liblensfun',
            # 其他开发库
            'libboost', 'libhdfs', 'libgsasl', 'libntlm',
            'libmetis', 'libscotch', 'libptscotch',
            'libsuitesparse', 'libcholmod', 'libumfpack',
            'libarpack', 'libparpack', 'libsuperlu',
            'libpastix', 'libmumps', 'libhypre', 'libpetsc',
            'libslepc', 'libtrilinos', 'libmoab',
            # 更多图像/多媒体
            'libgd', 'libheif', 'libhwy', 'libjxl', 'libopenal',
            'libplacebo', 'libpoppler', 'libqpdf', 'libqhull',
            'librsvg', 'librsvg2', 'librsvg2-common', 'librttopo',
            # 更多系统工具
            'libgc', 'libgc1c', 'libgdbm', 'libgdbm-compat', 'libgpm',
            'libgphoto', 'libgphoto2', 'libgs', 'libgudev',
            'libiec', 'libiec61883', 'libigdgmm',
            'libjpeg-turbo', 'libjs-jquery', 'libjs-underscore',
            'libkeyutils', 'liblcms', 'liblcms2',
            'liblockfile', 'libltdl', 'libmad', 'libmd', 'libmfx',
            'libmodplug', 'libmpcdec', 'libmpdec', 'libmpeg', 'libmpeg2',
            'libmpg', 'libmpg123', 'libmt', 'libmtdev',
            'libnotify', 'libnpth', 'libnspr', 'libnss', 'libnssutil',
            'libodbc', 'libodbccr', 'libodbcinst',
            'libosmesa', 'libpciaccess', 'libpgm',
            'libplc', 'libplds', 'libproxy', 'libpthread-stubs',
            'libpython', 'libpython3', 'libpython3-stdlib',
            'libqt5core', 'libsctp', 'libserd', 'libsord', 'libsratom',
            'libsz', 'libtdb', 'libtevent', 'libthai',
            'libtiffxx', 'libtirpc', 'libtool', 'libtool-bin',
            'libwrap', 'libxxhash', 'libz3', 'libzimg', 'libzvbi', 'libzvbi-common',
            # 更多压缩/编码
            'libaec', 'libb2', 'libbz', 'libdb', 'libdb5',
            'libde265', 'libdc', 'libdc1394',
            'libexif', 'libexpat', 'libgcrypt',
            'libgfortran-ng', 'libgfortran5',
            'libice', 'libidn11', 'libidn12',
            'libllvm', 'libllvm14',
            'libnghttp', 'libnorm', 'libnsl2',
            'libogg0', 'libopus0', 'liborc-0.4',
            'libp11-kit', 'libpq-dev',
            'libpulse0', 'libalsa2', 'libvorbis0a',
            'libvpx', 'libvpx7', 'libwebp6', 'libwebp7',
            'libx264', 'libx264-160', 'libx265', 'libx265-192', 'libx265-199',
            'libxcb1', 'libxcb-render', 'libxcb-shm',
            'libzstd1',
            # OPeNDAP / 科学数据协议
            'libdap', 'libdapclient', 'libdapserver', 'libopendap', 'libnc-dap',
            # 更多系统库 (A-Z)
            'libaio', 'libao', 'libapr', 'libaprutil', 'libart',
            'libasound', 'libasyncns', 'libatasmart', 'libavc1394',
            'libbind', 'libbinutils', 'libbio', 'libbison',
            'libbluetooth', 'libbluray', 'libbonobo', 'libbonoboui',
            'libboot', 'libbsd', 'libc6', 'libcaca',
            'libcanberra', 'libcap2', 'libcddb', 'libcdio',
