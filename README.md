...
- 🧩 **智能跳过** - 已安装且版本相同或更新的包自动跳过，避免重复安装
- 📈 **实时进度** - 恢复过程中显示百分比进度，清晰掌握安装状态
- 🔍 **恢复报告** - 恢复完成后生成详细对比报告，包含完整度评估
- 📝 **失败分析** - 15+ 类失败原因智能分析，提供针对性解决方案
- 📄 **日志导出** - 失败包自动保存日志文件，便于后续排查处理
- 📁 **多备份管理** - 支持同一环境多份备份，自动选择最新版本
...
- **智能跳过** - 已安装且版本相同或更新的包自动跳过
- **实时进度** - 显示百分比进度条，清晰掌握安装状态
- **恢复报告** - 完成后生成详细对比报告和完整度评估
- **失败分析** - 15+ 类失败原因智能分析，提供针对性解决方案
- **日志导出** - 失败包自动保存日志文件到 `_restore_logs/`
- **双恢复模式** - 支持备份版本恢复和最新版本恢复
...
### v0.3.0 (2026-05-01) - Mirror & Insight Edition
- ✨ **多镜像源支持** - 官方主站 + 6 大国内镜像源（清华/阿里云/中科大/腾讯云/华为云/豆瓣）
- ✨ **镜像源动态切换** - 命令行 `--mirror` 参数 + 交互式菜单，支持恢复时实时切换
- ✨ **实时失败原因分析** - 安装失败立即显示具体原因（系统库/网络/SSL/依赖冲突/权限等）
- ✨ **并行安装进度** - 批量安装显示实时百分比进度
- 🔧 优化交互式菜单，镜像源使用序号选择

### v0.2.0 (2026-05-01)
- ✨ **双恢复模式** - 支持备份版本恢复和最新版本恢复
  - 备份版本模式：严格按照备份版本安装，确保环境一致性
  - 最新版本模式：自动查询并安装最新版本，适合环境更新
- ✨ **智能跳过** - 已安装且版本相同或更新的包自动跳过，避免重复安装
- ✨ **实时进度** - 恢复过程中显示百分比进度条
- ✨ **恢复报告** - 完成后生成详细对比报告，包含完整度评估（优秀/良好/一般/较差）
- ✨ **失败分析** - 15+ 类失败原因智能分析（网络、编译、权限、版本冲突等）
- ✨ **日志导出** - 失败包自动保存日志文件到 `_restore_logs/`
- ✨ **多备份管理** - 支持同一环境多份备份，自动选择最新版本
- ✨ **递归查找** - 自动查找子目录中的备份文件
